/* Template: Arrays & Hashing.
 * Schema: { id, name, pattern, order, notes, code }. `code` is a template
 * literal so the Java inside stays readable and copy-pasteable.
 */
(window.LC_TEMPLATES = window.LC_TEMPLATES || {})['hashmap-count'] = {
  id: 'hashmap-count',
  name: 'Hash map: count / index / complement',
  pattern: 'arrays-hashing',
  order: 1,
  notes: 'Five shapes on one page. `count` tallies frequencies, `firstIndex` remembers where things were, ' +
         '`complement` is the Two Sum move, `groupByKey` buckets by a computed key, and the seen-set trick ' +
         'uses the return value of `add`. Whichever one a problem needs, the skeleton is identical: build the ' +
         'map as you go so a lookup replaces the inner loop.',
