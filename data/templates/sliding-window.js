(window.LC_TEMPLATES = window.LC_TEMPLATES || {})['sliding-window-variable'] = {
  id: 'sliding-window-variable',
  name: 'Sliding window: fixed k, longest valid, shortest valid',
  pattern: 'sliding-window',
  order: 3,
  notes: 'Three variants, and the difference is only WHERE you record the answer. Longest: record after the ' +
         'shrink loop, when the window is legal again. Shortest: record inside the shrink loop, while it is ' +
         'still legal. Fixed k: no shrink loop at all, just add one and drop one.',
