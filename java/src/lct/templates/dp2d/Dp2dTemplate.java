/*
 * TEMPLATE: 2-D DP: two-sequence table, grid paths, knapsack, row compression
 *
 * The padded row 0 and column 0 mean "empty prefix" and must be initialised on
 * purpose. Fill top-left to bottom-right and each cell only reads neighbours
 * that are already done. When a row depends only on the row above, collapse to
 * two rows.
 *
 * Generated from data/templates/dp-2d.js.
 */
package lct.templates.dp2d;
