/*
 * TEMPLATE: Binary search: exact, leftmost, on the answer
 *
 * Two loop idioms, and mixing them is the classic bug. while (lo <= hi) pairs
 * with hi = mid - 1 and returns from inside. while (lo < hi) pairs with hi =
 * mid and returns lo at the end. The third shape is the one that unlocks hard
 * problems: search the ANSWER range, not an array.
 *
 * Generated from data/templates/binary-search.js.
 */
package lct.templates.binarysearch;
