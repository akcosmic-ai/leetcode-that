/*
 * TEMPLATE: Trees: DFS orders, level-order BFS, BST range check
 *
 * Every recursion here starts with the null base case. The three DFS orders
 * differ only in WHERE you touch the node. The BFS trick is capturing q.size()
 * before the inner loop so one outer iteration equals exactly one level.
 * Validating a BST needs an inherited RANGE, not a parent comparison.
 *
 * Generated from data/templates/trees.js.
 */
package lct.templates.trees;
